...
- After making any code changes, immediately run `uv run pytest`, `uv run ruff check`, and `uv run ruff format` so regressions, lint issues, and formatting problems are caught before handing work off.
