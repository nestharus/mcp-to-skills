...
This file summarizes agent-specific guidance and points at key Architectural Decision Records (ADRs) under `docs/adr/` for full context. For code style, typing, and tooling standards, see `docs/code-style-guide.md`. This file focuses on agent-specific workflow expectations and repository conventions.
...
- After making any code changes, immediately run `uv run pytest` and `uv run lint` so regressions, lint issues, formatting problems, and type errors are caught before handing work off. Ruff is the standardized lint/format tool as described in `docs/adr/0005-standardize-on-ruff.md` and `docs/code-style-guide.md`, and mypy is enforced as part of the `uv run lint` pipeline.
